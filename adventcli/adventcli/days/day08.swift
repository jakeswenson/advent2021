...
let day08 = problem(day: 8) { input in
  let notes: [(signals: [String.SubSequence], outputs: [String.SubSequence])] = input.lines.map {
    line in
  ...
  part2(example: 61229, answer: 940724) {
    ...
    func matchDigit(_ chars: [Set<Character>], by: (Set<Character>) -> Bool) -> (
      Set<Character>, Array<Set<Character>>.SubSequence
    ) {
      var chars = chars
    ...
    let result = notes.map { note in
      ...
      let signalMap = Dictionary(
        uniqueKeysWithValues: note.signals
          .filter { numberLengths.contains($0.count) }
          .map { ($0.count, $0) })
